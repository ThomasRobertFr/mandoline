...
public class MetadataTest {
    ...
    public void testGetSchemas() {
        ...
        try {
            JSONObject jsonObjectTest = (JSONObject) (getSchemas.invoke(metadata));

            assertTrue("getJSONCube did not return the list of Schemas ", jsonObjectTest.has("Traffic"));
            assertEquals("getJSONCube did not return the Schemas captions ", "Traffic",
                         jsonObjectTest.getJSONObject("Traffic").getString("caption"));
    ...
    public void testGetDimensions() throws Throwable {
        ...
        try {
            ...
            assertEquals("the Dimension Zone does not have the type Geometry", result.getJSONObject("[Zone]").getString("type"), "Geometry");
            assertTrue("the result does not contain the second Dimension name", result.has("[Measures]"));
            assertEquals("the Dimension Zone does not have the type Geometry", result.getJSONObject("[Measures]").getString("type"),
                         "Measure");
            assertTrue("the result does not contain the third  Dimension name", result.has("[Time]"));
            assertEquals("the Dimension Zone does not have the type Geometry", result.getJSONObject("[Time]").getString("type"), "Time");
    ...
    public void testGetLevelProperties() {
        ...
        try {
            Level levelTest = olapConnection.getOlapCatalog().getSchemas().get("Traffic").getCubes().get("Traffic").getDimensions()
                                            .get("Zone").getHierarchies().get("Zone.Name").getLevels().get("Name3");

            JSONObject result = (JSONObject) (getLevelProperties.invoke(metadata, levelTest));

            assertTrue("the result does not contain the property Traffic Cube - Zone.Name Hierarchy - Name3 Level - Geom Property",
                       result.has("Traffic Cube - Zone.Name Hierarchy - Name3 Level - Geom Property"));
            assertEquals("the result does not contain a geometric property with the Geometry type",
                         result.getJSONObject("Traffic Cube - Zone.Name Hierarchy - Name3 Level - Geom Property").get("type"), "Geometry");

        } catch (JSONException | IllegalAccessException | IllegalArgumentException | OlapException | InvocationTargetException e) {
            e.printStackTrace();
        }
    ...
    public void testGetMemberProperties() {
        ...
        try {
            ...
            Member memberTest = olapConnection.getOlapCatalog().getSchemas().get("Traffic").getCubes().get("Traffic").getDimensions()
                                              .get("Zone").getHierarchies().get("Zone.Name").getLevels().get("Name3").getMembers().get(1);
            ...
            assertEquals("the result does not contain the geometric property value in WKT format of Nordburgenland, the first member of Name3 ",
                         result.get("Traffic Cube - Zone.Name Hierarchy - Name3 Level - Geom Property"), geometry);
    ...
    public void testGetMembers() throws Throwable {
        ...
        try {
            ...
            assertTrue("the result does not contain the first member's properties",
                       result.getJSONObject("[Zone.Name].[All Zone.Names].[Croatia]")
                             .has("Traffic Cube - Zone.Name Hierarchy - Name0 Level - Geom Property"));
            assertTrue(" the result does not contain the last member's properties  ",
            ...
            assertTrue("the result does not only contains the France member",
                       !result.has("[Zone.Name].[All Zone.Names].[Spain]") && result.has("[Zone.Name].[All Zone.Names].[France]"));
            result = (JSONObject) (getMembers.invoke(metadata, root, false, 2));
            assertTrue("the result does not only contains the children of France with a granularity 2",
                       result.has("[Zone.Name].[All Zone.Names].[France].[DÉPARTEMENTS D'OUTRE-MER].[Guyane]")
    ...
    public void testGetGeometry() {
        try {
            ...
            String result = (String) (getGeometry.invoke(metadata, root, memberTest, "Geom"));

            assertEquals("the result is not the geometric property value in WKT format of Nordburgenland, the first member of Name3 ",
                         result, geometry);
